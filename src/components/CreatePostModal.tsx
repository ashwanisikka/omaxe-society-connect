...
// STRICT RULES: App ID aur correct database paths rules configuration
const appId = "omaxe-heights-portal";

export const postService = {
  // 1. Sabhi society noticeboard posts ko naye se purane ke order mein fetch karta hai
  async getAllPosts() {
    // STRICT RULE 1: Direct root collection 'posts' block hai, correct path use karna padega
  ...
  // 3. Database mein naya post create karta hai jisme default status pending aur standard JS Date use hoti hai
  async createPost(
    title: string, 
    content: string, 
    category: string, 
    ...
    const newPost = {
      ...
      comments: [],
      createdAt: new Date() // JS Date so Firebase can native-convert to Timestamp safely
    };

    // STRICT PATH WRITE: Security Rules only allow writing here
    const postsRef = collection(db, 'artifacts', appId, 'public', 'data', 'posts');
